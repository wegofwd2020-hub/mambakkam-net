...
> **Known issue — deferred at 2026-05-17 launch.** Gmail forced the SMTP-server form on us even with "Treat as an alias" checked: after entering name + email + alias-checked and clicking Next Step, Gmail jumped to "Send mail through your SMTP server" with `route2.mx.cloudflare.net:587` prefilled. Cloudflare Email Routing has no outbound SMTP (inbound-only relays), so there's no password to enter — the form is a dead end. This is the §4.1 Fallback scenario; Google has been quietly retiring the "Send through Gmail" alias path for non-Workspace domain aliases since ~2024. The mambakkam.net send-as in §4 still worked on the same day, so this is per-domain (possibly domain-age or Cloudflare-Routing-flavour heuristic on Google's side).
>
> **Decision:** deferred `support@`/`sales@` send-as past Day 0 launch. Both inbound rules in §7.5 work (Cloudflare forwards incoming to Gmail), only outbound-as is missing. Day 0 cutovers (mambakkam 09:00 EDT, StudyBuddy 13:00 EDT) don't trigger outbound from these aliases.
>
> **Fix path when send-as becomes needed** (first contact-form submission, first paying customer, or first time you need to reply _as_ `support@`):
>
> - **Zoho Mail Lite** (~$12/yr) — real SMTP + DKIM aligned to the domain → DMARC tightens to `p=quarantine`
> - **Migadu** (~$19/yr) — similar
> - **Brevo free SMTP relay** — 300 emails/day; uses Brevo's DKIM, doesn't align to the domain (DMARC stays `p=none`)
> - **Google Workspace** ($6+/mo) — full Gmail UX + SMTP + DKIM for the domain
>
> Pick provider → swap §7's Cloudflare Email Routing for provider mailbox + provider SMTP → redo §7.6 with provider's SMTP creds (port 587, App Password, no alias mode needed) → update DMARC in §7.5.
...
| _SMTP_HOST / SMTP_USER / SMTP_PASSWORD_ | **Omit for demo.** No app-originated email ships in the demo; outbound from `siva@mambakkam.net` goes via Gmail send-as (alias mode), not from the server. When you later add a contact form + paid mail provider, paste back the four SMTP\_\* lines with provider host/credentials. |
...
| 2026-05-16 | 1.4     | **Stripe + Sentry deferred from tonight.** Stripe — verified in StudyBuddy*OnDemand `config.py`: `STRIPE_SECRET_KEY: str \| None = None` with all usage sites doing `getattr(..., None)`, raising only when a billing endpoint is hit. Demo flow (admin-provisioned Teacher + Student accounts browsing pre-built content) never touches billing, so app boots fine without keys. §9.1–§9.3 deferred to Phase 5. Sentry — replaced by Grafana Cloud Loki (§5) for error visibility; §9.4–§9.5 dropped. §9 collapses to the original §9.6 go/no-go (renamed to just "§9 — Final go/no-go"). §10 env-var table marks `STRIPE*\*`and`SENTRY_DSN` as "omit for demo". TL;DR row §9 retitled. Total time: 3.5h → ~3h.                                                                                                                                                                                                                                                                                                                                                            |
| 2026-05-17 | 1.5     | **§7.6 Gmail send-as for support@usestudybuddy.com deferred — Gmail force-routed to SMTP form despite alias-checked.** Hit during Day 0 morning operator walk-through. Even with "Treat as an alias" checked, Gmail's Next-Step jumped to "Send mail through your SMTP server" with `route2.mx.cloudflare.net:587` prefilled. Cloudflare Email Routing is inbound-only — no SMTP password exists, so the form is unresolvable as-is. The mambakkam.net send-as in §4 worked on the same morning, so this is per-domain (likely Google's growing reluctance to use the "Send through Gmail" alias path for non-Workspace domains with MX records, possibly age-gated for new domains). Inbound (§7.5 routing rules) is unaffected — Cloudflare still forwards `support@`/`sales@` to personal Gmail. Outbound-as deferred to post-launch when a paid mail provider with real SMTP + domain-aligned DKIM replaces Cloudflare Email Routing. Fix-path candidates documented in §7.6 (Zoho Mail Lite $12/yr, Migadu $19/yr, Brevo free SMTP relay, or Google Workspace $6+/mo). |
